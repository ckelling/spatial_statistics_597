...
Below, we see that the points on the plot represent the nonparametric variogram estimate and the line representes the fitted exponetnial variogram, using weighted least squares. There seems to be some structure to the variogram, and the exponential seems to be a decent fit. 
...
We also see that the standard error ($se = \sqrt(variance)$) also has some spatial dependence. In the last image, I have included the observation points on the image. We see that the variance is the highest where we don't have observation points, especially on the lower east side of California.
