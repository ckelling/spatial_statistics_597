...
I have made a couple of different choices in creating the simulation envelopes. First, I have set teh global option = FALSE for these envelopes because if I set global = TRUE, I get a smoother function but a much more conservative estimate of complete spatial randomness. I will also set nrank = 1 so that I create the envelope under the highest and lowest values of the function simulations. I will also increase the number of simlations to 500 so that my results aren't impacted by a small number of simulations. 

I am somewhat concerned that these bands will always be too wide with the global =TRUE (compared to global = FALSE option) so that I will always fail to reject the null hypothesis of complete spatial randomness. We see, for example, that the bands are quite wide around r=0 or a radius of 0, so that we shouldn't reject the null hypothesis of CSR if there are many points within a radius of 0. 
...
#    F function
f_aff <- envelope(aff_ppp, fun = Fest, global = FALSE, nrank = 1, nsim = 500)

#    G function
g_aff <- envelope(aff_ppp, fun = Gest, global = FALSE, nrank = 1, nsim = 500)


#creating MC sim envelope for unaffected dataset
#    F function
f_unaff <- envelope(unaff_ppp, fun = Fest, global = FALSE, nrank = 1, nsim = 500)

#    G function
g_unaff <- envelope(unaff_ppp, fun = Gest, global = FALSE, nrank = 1, nsim = 500)
...
\textbf{Can you explain the reason for this, based on the form of the test statistics?}
...
#creating MC sim envelope for affected dataset
#    F function
f_aff_2 <- envelope(aff_ppp_poly, fun = Fest, global = FALSE, nrank = 1, nsim = 500)
#    G function
g_aff_2 <- envelope(aff_ppp_poly, fun = Gest, global = FALSE, nrank = 1, nsim = 500)


#creating MC sim envelope for unaffected dataset
#    F function
f_unaff_2 <- envelope(unaff_ppp_poly,fun = Fest, global = FALSE, nrank = 1, nsim = 500)
#    G function
g_unaff_2 <- envelope(unaff_ppp_poly, fun = Gest, global = FALSE, nrank = 1, nsim = 500)
